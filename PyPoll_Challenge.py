...
with open(file_to_save, "w") as txt_file:
    ...
    for county_name in county_votes:
        ...
        percentage_county = float(county)/float(total_votes) * 100
        county_results = (
            f"{county_name}: {percentage_county:.1f}% ({county:,})\n")
        ...
        if (county>winning_county) and (percentage_county > winning_county_percentage):
            winning_county = county
            winning_county_candidate = county_name
            winning_county_percentage = percentage_county
